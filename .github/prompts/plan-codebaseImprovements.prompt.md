...
16. Document optional feature dependencies (`Microsoft.Graph.Authentication`, `IntuneWin32App`, `Az.Accounts`, `Az.Resources`, and `Az.Storage`) separately from the required Evergreen module, expose a prerequisite checker with actionable messages, and list optional modules in manifest metadata where supported. Correct the README resource typo from `evergreenbulk.png` to `evergreenbulb.png`.
